...
/** Achievements list — embeds into the level modal (or a standalone modal). */
export function AchievementsPanel({ onApplesClaimed }: PanelProps) {
  ...
  if (loading) {
    return <p className="xp-history-empty">Загрузка…</p>;
  }

  return (
    <div className="ach-tiers" data-achievements-panel="true">
      {tiers.map(tier => (
...
/** Standalone achievements modal (legacy entry). Prefer embedding AchievementsPanel. */
export default function AchievementsModal({ onClose, onApplesClaimed }: ModalProps) {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <AchievementsPanel onApplesClaimed={onApplesClaimed} />
      </motion.div>
    </motion.div>
  );
}
