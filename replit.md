...
**Росток** — это игровой интерфейс поверх накопительного счёта, создаваемый с разрешения клиента банка. Приложение отслеживает рост вклада и превращает ежедневный уход за капиталом в привычку — по аналогии с тем, как Duolingo превратил изучение языков в ежедневный ритуал.

Идея простая: **Duolingo в мире накопительных счетов.** Так же, как языковое приложение превращает скучную учёбу в ежедневный ритуал — Росток превращает слежение за вкладом в привычку. Вы открываете приложение каждый день не потому что «надо», а потому что интересно посмотреть, как вырос ваш капитал и дерево. В результате формируется финансовая дисциплина: привычка сберегать, регулярно пополнять вклад и следить за ростом накоплений.
...
- 💧 **Вода** — капли падают сверху, нужно двигать корзину и ловить как можно больше. Чем выше процент попаданий, тем лучше результат.
- ☀️ **Свет** — солнечные лучи появляются в случайных местах экрана, нужно нажимать на них пока не закончится время. Скорость решает.
- 🍃 **Листва** — листочки падают сверху рядами, нужно собирать три одного цвета подряд. Чем длиннее серия, тем выше очки.

---

### Дерево растёт вместе с капиталом
...
В конце каждой сессии дерево растёт в прямом эфире — игрок видит анимацию роста и получает яблоки (символы дохода), которые нужно собрать вручную. Это небольшой ритуал, который делает каждый день запоминающимся.
...
**🌿 Базовый — 12% годовых.** Начисляется каждый день автоматически — независимо от того, насколько хорошо сыграл игрок. Это гарантированная часть дохода, которую получаешь просто за то, что проводишь сессию.

**⭐ Бонусный — до +3% годовых.** Зависит от результата в мини-играх, размера капитала и регулярности сессий. Чем точнее играешь и чем реже пропускаешь — тем выше бонус. При длинной серии без пропусков бонусный множитель сохраняется на максимуме.

**⚡ Супер-сессия.** Пропущенные дни не сгорают: сессии накапливаются, и при следующем входе игрок получает всё сразу — с повышенным множителем. Вернулся через три дня? Один вход закроет все пропуски.
...
- **Меньше оттока средств.** Чем больше клиентов вовлечены и видят прогресс, тем реже они снимают накопленное.
- **Монетизация внимания.** Ежедневное присутствие клиента в приложении — ценный актив: партнёрские предложения, кросс-продажи финансовых продуктов внутри игрового контекста.
- **Лояльность и вовлечённость.** Игровой формат создаёт эмоциональную привязанность к бренду банка.

---

### Модель партнёрства через API накопительных счетов
...
**Схема работы:**
1. Клиент авторизует доступ к своему накопительному счёту через OAuth-протокол банка (по аналогии с Open Banking / ПСД2)
2. Росток подключается к API банка и читает реальный баланс: дерево растёт вместе с настоящими накоплениями, доход начисляется по реальным ставкам банка
3. Банк получает ежедневно вовлечённых клиентов, которые реже снимают средства

**Техническая интеграция:** OAuth 2.0, Open Banking-совместимые интерфейсы или прямой договор с банком на доступ к API накопительных счетов.
